...
let make_create_function_object_call x_sc fun_id params =
  ...
  let cmd = LCall (x_f, Lit (String createFunctionObjectName),
    [ (PVar x_sc); (Lit (String fun_id)); (Lit (String fun_id)); (Lit (LList processed_params)) ], None, None) in
  x_f, cmd
...
let rec translate_expr tr_ctx e : ((Annot.t * (string option) * LabCmd.t) list) * Expr.t * (string list) =
  ...
  (* Printf.printf "NOW NOW NOW lcmds: %s\n call_cmds: %s with the expression %s\n"
    ...
    (JSPrettyPrint.string_of_exp true e); *)
...
*)
  ...
  *)
    ...
    *)
    ...
    let translate_accessor_descriptor x_obj prop accessor is_getter err =
      ...
      let params =
        ...
        | JSParserSyntax.FunctionExp (_, _, params, _) -> params
  ...
  | JSParserSyntax.FunctionExp (_, None, params, e_body) ->
    (**
       Section 13
       x_f := create_function_object(x_sc, f_id, f_id, params)
    *)
  ...
  | JSParserSyntax.FunctionExp (_, Some f_name, params, _)
  | JSParserSyntax.Function (_, Some f_name, params, _) ->
    let f_id = try JS2JSIL_Preprocessing.get_codename e
...
and translate_statement tr_ctx e  =
  ...
  | JSParserSyntax.Function (_, n, params, e_body) -> [], Lit Empty, [], [], [], []
  ...
  let hoisted_fdecls =
    List.fold_left (fun ac f_decl ->
      let f_name, f_params =
        ...
        | JSParserSyntax.Function (s, Some f_name, f_params, body) -> f_name, f_params
...
let generate_main offset_converter e spec : EProc.t =
  ...
  let cmd_del_te = annotate_cmd (LCall (x_ignore, Lit (String JS2JSIL_Constants.deleteObjAndMetadata), [ PVar var_te ], None, None)) None in
  let cmd_del_se = annotate_cmd (LCall (x_ignore, Lit (String JS2JSIL_Constants.deleteObjAndMetadata), [ PVar var_se ], None, None)) None in 
  ...
  let main_cmds =
    ...
    cmds_hoist_fdecls @
    cmds_e @
    [ret_ass; cmd_del_te; cmd_del_se; lab_ret_cmd ] @ 
     err_cmds in
  {
    name = main_fid;
...
let generate_proc ?use_cc offset_converter e fid params vis_fid spec : EProc.t =
  let annotate_cmd cmd lab = (Annot.init (), lab, cmd) in
  ...
  *)
  ...
  let cmds_arg_obj =
    [
      ...
      (Annot.init (), None, LBasic (Assignment (x_argList_act, UnOp (Cdr, (UnOp (Cdr, PVar x_argList_pre))))));
  ...
  let cmd_del_te    = annotate_cmd (LCall (x_ignore, Lit (String JS2JSIL_Constants.deleteObjAndMetadata), [ PVar var_te ], None, None)) None in
  let cmd_del_se    = annotate_cmd (LCall (x_ignore, Lit (String JS2JSIL_Constants.deleteObjAndMetadata), [ PVar var_se ], None, None)) None in 
  (* let cmd_del_args  = annotate_cmd (LCall (x_ignore, Lit (String JS2JSIL_Constants.deleteObjAndMetadata), [ PVar var_args ], None)) None in *)
  ...
  let fid_cmds =
    ...
    [ cmd_dr_ass; cmd_return_phi; cmd_del_te; cmd_del_se ] @ 
    (if_verification [] [ ]) @ 
    [ cmd_xscope_final; cmd_ret_final; cmd_error_phi; cmd_xscope_final] @
    (if_verification [] [ cmd_del_errs ]) @ [ cmd_err_final ] in
  {
    name = fid;
    body = (Array.of_list fid_cmds);
...
(**** EVAL ****)
let js2jsil_eval (prog : Prog.t) fid_parent e =
...
let compute_imports (for_verification : bool) : string list =  
  if for_verification 
    then js2jsil_logic_imports 
    else if !(SCommon.bi) 
      then js2jsil_imports_bi
      else if !(SCommon.cosette)
...
let js2jsil e offset_converter for_verification =
  let e, only_specs, predicates, ids = JS2JSIL_Preprocessing.preprocess cc_tbl fun_tbl vis_tbl e in
  ...
  (EProg.init imports lemmas predicates only_specs procedures macros bispecs (ids @ [ "main" ])), cc_tbl, vis_tbl
