...
let check_satisfiability_with_model 
  ...
  L.log L.Verboser (lazy (Printf.sprintf "OBTAINED VARS: %s\n" (String.concat ", " (SS.elements z3_vars)))); 
  (match model with
  | None -> None 
  | Some model -> try (Z3Encoding.lift_z3_model model gamma subst z3_vars; Some subst) with _ -> None)
...
let check_entailment 
  ...
  (* If right side is false, return false *)
    ...
    if ((List.length right_fs = 0) || not left_sat) then left_sat else (
      ...
      L.log L.Verbose (lazy (Printf.sprintf "Entailment returned %b" (not ret)));
      not ret 
    )
  )
